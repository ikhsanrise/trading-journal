...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const setup = await prisma.setup.findFirst({ where: { id: params.id, userId: user.id } });
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await prisma.trade.updateMany({ where: { setupId: params.id }, data: { setupId: null } });
  await prisma.setup.delete({ where: { id: params.id } });
